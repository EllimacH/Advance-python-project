...
class AdminCLI(User):
    ...
    def delete_user(self) -> None:
        ...
        for user in self.system.users:
            ...
            if user.username == username:
                confirm = input(
                    f"Are you sure you want to delete {user.username} account? (y/N): "
                )
                if confirm.lower() != "y":
                    return
    ...
    def change_balance(self) -> None:
        username: str = input(
            "Enter username of the user whose balance you want to change: "
        )
        ...
        # change the balance
        operation: str = input(
            "Do you want to add or subtract from the balance? (+/-): "
        )
        match operation:
            case "+":
    ...
    def add_mobile_plan(self) -> None:
        ...
        self.system.mobile_plans[plan_id] = {
            "id": plan_id,
            "name": plan_name,
            "price": plan_price,
            "gb": plan_gb,
    ...
    def remove_mobile_plan(self) -> None:
        ...
        for plan_id, plan in self.system.mobile_plans.items():
            print(
                f"{plan_id} : {plan['name']}, {plan['price']}, {plan['gb']}, {plan['description']}"
            )
        ...
        for key, plan in self.system.mobile_plans.items():
            ...
            if key == mb_plan_id:
                confirm = input(
                    f"Are you sure you want to delete {plan['name']} plan? (y/N): "
                )
                if confirm.lower() != "y":
                    return
