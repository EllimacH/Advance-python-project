...
class Menu:
    ...
    def is_not_logged_in(self) -> bool:
        ...
        while True:
            ...
            if is_first_account:
                print("-" * 10)
                print(
                    "There is no account in the system, this first account will be an admin account"
                )
            choice = input("\nEnter your choice: ")
            match choice:
                case "1":
                ...
                case "2":
                    return (
                        self.system.log_in()
                    )  # Return True if logged-in successfully to go to `is_signed_in()` menu, False otherwise to stay in this menu.
                case _:
    ...
    def is_signed_in(self) -> bool:
        ...
        while True:
            ...
            self.system.clear_screen()
            print(
                f"== Welcome {self.system.logged_in_user.username}! =="
            )  # self.system.users contains list of User objects, after signing in, that user object is assigned to self.system.logged_in_user, so we can access its attributes conveniently, like the `username` here.
            ...
            match choice:
                case "1":
                    self.system.check_balance()
                case "2":
                    self.system.list_mobile_plans()
                case "3":
                    self.system.register_mobile_plan()
                case "4":
                    self.system.check_current_plan()
                case "5":
                    self.system.top_up()
                case "6":
                    self.web.web_domain_service_menu()
                case "7":
                    return (
                        self.system.log_out()
                    )  # Here's the exit point of this method's loop. `log_out()` return False, assign to `is_logged_in` in `main()` function to get back to `is_not_logged_in()` menu.
                case "8":
                    self.system.transaction_history()
                case "*":
                    if self.system.logged_in_user.is_admin:
